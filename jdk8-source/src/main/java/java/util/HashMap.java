 ...
 * unsynchronized and permits nulls.非同步)  This class makes no guarantees as to
 * the order of the map; in particular, it does not guarantee that the order
 * will remain constant over time.不能保证加入的次序保持不变
 * 
 * 对于基本的get put操作，能够保持不变的时间复杂度（hash函数保证分散度足够的情况）
 * <p>This implementation provides constant-time performance for the basic 
 * operations (<tt>get</tt> and <tt>put</tt>), assuming the hash function
 * disperses the elements properly among the buckets. 
 * 迭代遍历需要的时间与HashMap中实例数量（即桶的数量） + 键值对的大小 呈正比
 * Iteration over collection views requires time proportional to the "capacity" of the
 * <tt>HashMap</tt> instance (the number of buckets) plus its size (the number
 * of key-value mappings). 
 * 因此，初始的容量不应该太高或者太低
 * Thus, it's very important not to set the initial capacity too high (or the load factor too low) if iteration performance is
 * important.
 * HashMap 中两个重要的参数：“初始容量” 和 “加载因子”。 
 * <p>An instance of <tt>HashMap</tt> has two parameters that affect its
 * performance: <i>initial capacity</i> and <i>load factor</i>.  
 * 初始容量：是哈希表中桶的数量，初始容量 只是哈希表在创建时的容量
 ...
 * number of buckets.
 *
 * 作为一个常见规则，加载因子0.75 是时间与空间消耗权衡的结果
...
public class HashMap<K,V> extends AbstractMap<K,V>
     ...
     */
    static final float DEFAULT_LOAD_FACTOR = 0.75f;   //初始装载因子0.75
    ...
    static class Node<K,V> implements Map.Entry<K,V> {
        ...
        //异或运算 ^ 100^010 = 110 相异为1
        public final int hashCode() {
            return Objects.hashCode(key) ^ Objects.hashCode(value);  //计算hashcode的方法，异或运算^
        }
        ...
        public final boolean equals(Object o) {
            if (o == this)  //先判断引用
                return true;
            if (o instanceof Map.Entry) {  //再判断类型
    ...
    final Node<K,V>[] resize() {
        Node<K,V>[] oldTab = table;  //oldTable 为当前表的哈希桶
